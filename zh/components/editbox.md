...
在 v2.1 里，我们对 EditBox 组件做了一些调整和优化。主要包括：
...
- 废弃 `setFocus()` 方法，新增 `focus()` 和 `blur()` 方法，给 `CCEditBox` 组件提供主动结束编辑的能力
...
说明：在用户按下回车键的时候，或者在移动端点击软键盘上的完成按钮时候，该事件会被触发。  
